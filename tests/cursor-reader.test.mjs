...
test('Cursor IDE reads explicit states and public bubbles and maps only a known workspace', () => fixture(({ userData, home, root, now }) => {
  ...
  try {
    ...
    insert(db, 'composerData:enum', { status: 2, lastUpdatedAt: now });
    const result = readCursorSnapshot({ home, userData, now });
    assert.equal(result.connected, true); assert.equal(result.error, null); assert.equal(result.tasks.length, 3);
    ...
    assert.equal(live.title, 'Live task'); assert.deepEqual(live.jumpTarget, { kind: 'project', app: 'Cursor', cwd: join(root, 'My project') });
    ...
    assert.equal(result.tasks.find(t => t.nativeId === 'enum').jumpTarget, undefined);
    assert.equal(db.prepare('SELECT count(*) AS n FROM cursorDiskKV').get().n, 6);
  } finally { workspace.close(); db.close(); }
}));
test('Cursor stale generation is unconfirmed and damaged records do not hide valid records', () => fixture(({ home, userData, now }) => {
  const db = dbAt(join(userData, 'globalStorage', 'state.vscdb'));
  try {
    ...
    assert.equal(result.tasks.find(t => t.nativeId === 'broken').status, 'unknown');
    assert.equal(result.tasks.find(t => t.nativeId === 'unrecognized').status, 'unknown');
    assert.equal(readCursorSnapshot({ home, userData, now, limit: 1 }).tasks.length, 1);
  } finally { db.close(); }
}));
test('Cursor selected and last-focused composer indexes map projects without inventing metadata', () => fixture(({ home, userData, root, now }) => {
  ...
  try {
    ...
    for (const entry of result.tasks) assert.equal(entry.jumpTarget.cwd, join(root, 'Mapped project'));
...
test('Cursor CLI requires explicit result and excludes reasoning and tool outputs', () => fixture(({ home, userData, root, now }) => {
  ...
  assert.equal(done.status, 'completed'); assert.equal(done.sourceLabel, 'Cursor · CLI'); assert.equal(done.jumpTarget.cwd, join(root, 'project'));
...
test('Cursor missing data is disconnected; damaged IDE does not block a valid CLI', () => fixture(({ home, userData, now }) => {
  assert.equal(readCursorSnapshot({ home, userData, now }).connected, false);
