...
function fixture(t) {
  ...
  return { root, home, userData, cwd, now, write, event, session, read: () => readKiroSnapshot({ home, userData, now }) };
}
...
test('v2 lifecycle requires an explicit successful turn_end and returns no transcript content', t => {
  ...
  assert.deepEqual(before.tasks[0].jumpTarget, { kind: 'project', app: 'Kiro', cwd: f.cwd });
...
test('missing source disconnects; invalid metadata, absent paths and unsupported formats remain conservative', t => {
  ...
  assert.equal(snapshot.tasks[0].jumpTarget, null);
});
