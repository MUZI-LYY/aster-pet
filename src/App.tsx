...
export default function App(){
  ...
  return <main className="popover">
    ...
    {page==='tasks'?<><div className="summary status-summary">{visibleStatuses.map(status=><button key={status} className={`summary-status ${status}`} aria-pressed={filter===status} onClick={()=>selectFilter(status)}><b>{counts[status]}</b><span>{statusLabels[status as keyof typeof statusLabels]}</span></button>)}</div>
      ...
      <div className="task-list" aria-label="本机任务列表">{tasks.map(t=>{const content=<><span className={`task-dot ${t.status}`}/><span className="task-content"><span className="task-top"><b>{t.title}</b><span className={`status ${t.status}`}>{labels[t.status]||'待核实'}</span></span>{t.subtitle&&<span className="task-subtitle" title={t.subtitle}>{t.subtitle}</span>}<span className="task-meta"><span className="task-source" title={`${t.sourceLabel||t.provider||'Codex'} · ${t.project}`}>{t.sourceLabel||t.provider||'Codex'} · {t.project}</span><span className={`task-tokens ${t.tokenUsage?'':'unavailable'}`} title={tokenDescription(t.tokenUsage)}>{tokenLabel(t.tokenUsage)}</span><span className="task-time">{timeAgo(t.updatedAt)}</span></span>{needsYou(t.status)&&<span className="attention-note">{t.status==='approval'?'需要你批准后才能继续':t.status==='input'?'正在等待你的回答':'需要你确认'}</span>}</span></>;return t.jumpTarget?<button key={t.id} className="task-row" title={`${t.evidence}\n${t.jumpTarget.label||'点击定位'}`} onClick={async()=>{if(!window.aster){setNotice('任务跳转请使用桌面版');return;}try{const result=await window.aster.openTask(t.id);setNotice(result.message||'');}catch{setNotice('无法打开任务，请回到对应工具查看');}}}>{content}<ArrowUpRight size={13} className="jump-icon"/></button>:<div key={t.id} className="task-row no-jump" title={`${t.evidence}\n暂不支持直达原会话`}>{content}<span className="no-jump-note">不可直达</span></div>;})}{!tasks.length&&<div className="empty">{snapshot.checkedAt?'暂无匹配任务':'正在读取本机任务…'}<small>{snapshot.sources?.find(s=>s.provider===provider)?.error||snapshot.error||(provider==='cli'?'使用 Aster CLI 包装命令启动任务后会显示在这里':'开始任务后会自动出现在这里')}</small></div>}</div>
      <footer><i className={snapshot.connected?'online':''}/><span>每 3 秒同步</span><span>可用任务点击定位</span></footer></>:<div className="settings">
